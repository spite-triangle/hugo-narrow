...
class TOCManager {
  constructor() {
    ...
    this.alwaysVisible = true; // 启用后始终展示，不自动隐藏
  ...
  init() {
    ...
    this.setupElements();
    // 没有任何可交互元素时无需继续初始化
    // （center/left 模式下 section 切换按钮与小节无关，即使无 TOC 链接也应绑定交互）
    if (
      this.tocLinks.length === 0 &&
      !this.sectionCenterToggle &&
      !this.sideIndicator
    ) {
      return;
    }
  ...
  bindEvents() {
    ...
    // Section Center Toggle 交互逻辑（与 centerToggle 相同模式）
    if (this.sectionCenterToggle) {
